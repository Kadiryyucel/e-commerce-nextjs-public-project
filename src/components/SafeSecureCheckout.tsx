...
export default function SafeSecureCheckout(props: { className?: string, showText?: boolean }) {

    const { className, showText = true } = props
        ...
        <>
            <div className={`flex flex-col justify-center gap-y-4 px-4 mt-8 h-32 bg-item ${className}`}>
                <div className='flex justify-center items-center w-full'>
                    {showText && <div><IoLockClosedOutline size={32} /></div>}
                    {showText && <div className='text-sm md:text-base xl:text-lg'>Guaranteed Safe & Secure Checkout</div>}
                </div>
                <div>
